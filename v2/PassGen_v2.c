...
uint_fast8_t is_valid_cond (uint_fast8_t *cond, uint_fast8_t l_pass);

/* Entry Point */
int main (void) {

    /* Variable definition */
    uint_fast8_t isn = 0;     // Valid password length flag. (0: invalid, 1: valid)
    uint_fast8_t l_pass;      // Password length (8 - 255).
    char         l_pass_a[3]; // Ascii password length to change l_pass. 
    char         *passwd;     // Password.
    char         *fp_pass;    // First pointer of passwd.
    uint_fast8_t *cond;       // Password character condition.
    uint_fast8_t *fp_cond;    // First pointer of cond.
    int_fast16_t i;           // for-loop index.
    ...
    */
    ...
    cond   = (uint_fast8_t *) malloc(l_pass);     // Password character condition
    ...
    */
    ...
    for (i = 0; i < l_pass; i++) {
        switch (cond[i]){
            case 1:
                /* A - Z */
                passwd[i] = (genrand64_int64() % 26) + 65;
                break;

            case 2:
                /* a - z */
                passwd[i] = (genrand64_int64() % 26) + 97;
                break;

            case 3:
                /* 0 - 9 */
                passwd[i] = (genrand64_int64() % 10) + 48;
                break;

            case 4:
                /* !, ", #, $, %, &, ', (, ), *, +, ,, -, ., / */
                passwd[i] = (genrand64_int64() % 15) + 33;
                break;
        ...
        cond[i] = 0; // Fill the condition number.
        //cond++;    // Next condition number index.
        //passwd++;  // Next password index.
    }

    passwd[l_pass] = '\0';   // Fill NULL letter.
...
uint_fast8_t is_valid_cond (uint_fast8_t *cond, uint_fast8_t l_pass) {
    ...
    //uint_fast8_t *fp_cond;  // First pointer of *cond
    uint_fast8_t cond1 = 0; // The number of condition 1
    uint_fast8_t cond2 = 0; // The number of condition 2
    uint_fast8_t cond3 = 0; // The number of condition 3
    uint_fast8_t cond4 = 0; // The number of condition 4
    int_fast16_t i;         // for-loop index
